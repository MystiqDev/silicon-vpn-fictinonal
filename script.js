...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.faq-tab').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('.faq-tab').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-selected', 'false'); });
      document.querySelectorAll('.faq-panel').forEach(p => p.classList.remove('active'));
      btn.classList.add('active');
      btn.setAttribute('aria-selected', 'true');
      const panel = document.querySelector('.faq-panel[data-panel="' + target + '"]');
  ...
  function createModal(content) {
    document.querySelectorAll('.svpn-modal-backdrop').forEach(m => m.remove());

    const backdrop = document.createElement('div');
    backdrop.className = 'svpn-modal-backdrop';
    backdrop.innerHTML = '<div class="svpn-modal" role="dialog" aria-modal="true">' + content + '</div>';
    ...
    backdrop.querySelectorAll('.svpn-modal-close, [data-close-modal]').forEach(btn => {
  ...
  function buildPlanModal(plan) {
    ...
    const p = plans[plan];
    const perksHTML = p.perks.map(function(item) {
      return '<li><svg viewBox="0 0 24 24" fill="none" stroke="' + p.color + '" stroke-width="2.5" width="14" height="14"><polyline points="20 6 9 17 4 12"/></svg>' + item + '</li>';
    }).join('');

    return '<button class="svpn-modal-close" aria-label="Close">&times;</button>' +
      '<div class="plan-modal-inner">' +
        '<div class="plan-modal-head">' +
          '<span class="plan-modal-badge ' + p.badgeClass + '">' + p.badge + '</span>' +
          '<h2 class="plan-modal-title">' + p.name + '</h2>' +
          '<div class="plan-modal-price">' + p.price + '</div>' +
          '<p class="plan-modal-desc">' + p.desc + '</p>' +
        '</div>' +
        '<ul class="plan-modal-perks">' + perksHTML + '</ul>' +
    ...
    return '<button class="svpn-modal-close" aria-label="Close">&times;</button>' +
      '<div class="card-form-inner">' +
        '<div class="card-form-head">' +
          ...
          '<div><h2>Secure Checkout</h2><p>' + planName + ' · <strong>' + price + '</strong></p></div>' +
        '</div>' +
        ...
        '<button class="btn btn-pay-now" id="btn-pay-now">' +
          '<span id="pay-btn-label"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="16" height="16"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>Pay ' + price + ' Securely</span>' +
  ...
  function initCardFormListeners(modal, planName, price) {
    ...
    numInput.addEventListener('input', function() {
      ...
      else if (v.startsWith('5') || v.startsWith('2')) networkEl.textContent = 'MC';
    ...
    modal.querySelector('#btn-pay-now').addEventListener('click', function() {
      ...
      if (!name)           { shakeField(nameInput);  showToast('Please enter your cardholder name.', 'error'); return; }
      if (num.length < 16) { shakeField(numInput);   showToast('Please enter a valid 16-digit card number.', 'error'); return; }
      if (exp.length < 7)  { shakeField(expInput);   showToast('Please enter the expiry date.', 'error'); return; }
      if (cvv.length < 3)  { shakeField(cvvInput);   showToast('Please enter your CVV.', 'error'); return; }
      if (!email || !email.includes('@')) { shakeField(emailInput); showToast('Please enter a valid email address.', 'error'); return; }
      ...
      setTimeout(function() {
        ...
        setTimeout(function() { showSuccessModal(planName, price, email); }, 350);
      }, 2000);
    });
  }
  ...
  function shakeField(input) {
    ...
    setTimeout(function() { input.classList.remove('shake'); input.style.borderColor = ''; }, 600);
  }
  ...
  function showSuccessModal(planName, price, email) {
    createModal(
      ...
      '<div class="success-modal-inner">' +
        ...
        '<p class="success-plan">' + planName + ' · ' + price + '</p>' +
        '<p class="success-msg">Your payment was processed successfully. A confirmation has been sent to <strong>' + email + '</strong>.</p>' +
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', function(e) {
      e.preventDefault();
      const content = 'SiliconVPN Installer — Basic (Free)\n' +
        '======================================\n' +
  ...
  document.querySelectorAll('.pcard-cta').forEach(function(btn) {
    ...
    btn.addEventListener('click', function(e) {
      ...
      const planNameText = (card.querySelector('.pcard-name') || {}).textContent || '';
      const plan  = planNameText.trim().toLowerCase() === 'premium' ? 'premium' : 'standard';
      ...
      backdrop.querySelector('#btn-paypal').addEventListener('click', function() {
        showToast('PayPal integration coming soon!', 'info');
      });

      backdrop.querySelector('#btn-card').addEventListener('click', function() {
        backdrop.classList.remove('open');
        document.body.classList.remove('no-scroll');
        setTimeout(function() {
          backdrop.remove();
          const { backdrop: cardBackdrop } = createModal(buildCardForm(title, price));
  ...
  function openSignupModal() {
    ...
    backdrop.querySelector('.toggle-pass').addEventListener('click', function() {
      const inp = backdrop.querySelector('#su-pass');
      inp.type = inp.type === 'password' ? 'text' : 'password';
    });

    backdrop.querySelector('#btn-create-account').addEventListener('click', function() {
      ...
      setTimeout(function() {
        ...
        showToast('Welcome, ' + fname + '! Your account is ready. \uD83C\uDF89', 'success', 5000);
      }, 1500);
    });

    backdrop.querySelector('#open-login').addEventListener('click', function(e) {
      e.preventDefault();
      backdrop.classList.remove('open');
      document.body.classList.remove('no-scroll');
      setTimeout(function() { backdrop.remove(); openLoginModal(); }, 250);
    });
  }
  ...
  function openLoginModal() {
    ...
    backdrop.querySelector('#btn-login').addEventListener('click', function() {
      ...
      setTimeout(function() {
        ...
        document.body.classList.remove('no-scroll');
        setTimeout(function() { backdrop.remove(); }, 300);
        showToast('Logged in successfully! Welcome back. \uD83D\uDC4B', 'success', 4000);
      }, 1200);
    });

    backdrop.querySelector('#forgot-pass').addEventListener('click', function(e) {
      e.preventDefault();
      showToast('Password reset link sent to your email.', 'info', 5000);
    });

    backdrop.querySelector('#open-signup-from-login').addEventListener('click', function(e) {
      e.preventDefault();
      backdrop.classList.remove('open');
      document.body.classList.remove('no-scroll');
      setTimeout(function() { backdrop.remove(); openSignupModal(); }, 250);
    });
  }
  ...
  function openContactModal() {
    ...
    backdrop.querySelector('#btn-send-msg').addEventListener('click', function() {
      ...
      if (!email || !email.includes('@')) { showToast('Please enter a valid email.', 'error'); return; }
      if (!subj)  { showToast('Please select a subject.', 'error'); return; }
      ...
      setTimeout(function() {
        ...
        document.body.classList.remove('no-scroll');
        setTimeout(function() { backdrop.remove(); }, 300);
        showToast("Message sent! We'll reply within 5 minutes. \u2713", 'success', 5000);
      }, 1000);
    });
  }
  ...
  document.querySelectorAll('.navbar-cta .btn-primary, .mobile-menu .mobile-nav-cta').forEach(function(btn) {
    btn.addEventListener('click', function(e) {
      e.preventDefault();
      closeMenu();
      openSignupModal();
  ...
  document.querySelectorAll('.fsplit-text .btn-primary[href="#pricing"]').forEach(function(btn) {
    btn.addEventListener('click', function(e) {
      e.preventDefault();
      document.getElementById('pricing').scrollIntoView({ behavior: 'smooth' });
      setTimeout(function() {
        document.querySelectorAll('.pcard').forEach(function(c, i) {
          setTimeout(function() { c.classList.add('pulse-once'); setTimeout(function() { c.classList.remove('pulse-once'); }, 600); }, i * 100);
        });
      }, 650);
    });
  });
  ...
  const unlockBtn = document.querySelector('.fsplit-text .btn-gold[href="#pricing"]');
  if (unlockBtn) {
    unlockBtn.addEventListener('click', function(e) {
      ...
      setTimeout(function() {
        ...
        if (prem) { prem.classList.add('pulse-once'); setTimeout(function() { prem.classList.remove('pulse-once'); }, 700); }
      }, 650);
    });
  }

  // FAQ & footer "Contact Support"
  document.querySelectorAll('.faq-cta-strip .btn-primary, a[href="#contact"].btn').forEach(function(btn) {
    btn.addEventListener('click', function(e) {
      e.preventDefault();
      openContactModal();
    });
  ...
  document.querySelectorAll('a[href="#pricing"]:not(.btn-primary):not(.btn-gold):not(.fsplit-text a)').forEach(function(a) {
  ...
  if (fmcCard) {
    const servers = [
      { flag: '\uD83C\uDDF3\uD83C\uDDF1', city: 'Amsterdam',  country: 'Netherlands',    ms: '12',  ip: '185.220.XXX.XXX' },
      { flag: '\uD83C\uDDFA\uD83C\uDDF8', city: 'New York',   country: 'United States',  ms: '38',  ip: '104.21.XXX.XXX'  },
      { flag: '\uD83C\uDDEC\uD83C\uDDE7', city: 'London',     country: 'United Kingdom', ms: '8',   ip: '51.89.XXX.XXX'   },
      { flag: '\uD83C\uDDE9\uD83C\uDDEA', city: 'Frankfurt',  country: 'Germany',        ms: '22',  ip: '95.213.XXX.XXX'  },
      { flag: '\uD83C\uDDF8\uD83C\uDDEC', city: 'Singapore',  country: 'Singapore',      ms: '110', ip: '45.76.XXX.XXX'   },
      { flag: '\uD83C\uDDEF\uD83C\uDDF5', city: 'Tokyo',      country: 'Japan',          ms: '88',  ip: '103.152.XXX.XXX' },
    ];
    let si = 0;
    setInterval(function() {
      ...
      if (ipEl)   ipEl.innerHTML   = s.ip.replace('XXX.XXX', '<span class="fmc-redact">XXX.XXX</span>');
